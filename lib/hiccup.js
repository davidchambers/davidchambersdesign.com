    ...
    case 'parenthesized': {
      ...
      if (term.elements[0].type === 'identifier' &&
        ...
        if (predicate.value) {
          ...
          const alternative = evaluate (env)
                                       (term.elements[3])
                                       (value => ({success: false, value}))
                                       (value => ({success: true, value}));
      ...
      if (term.elements[0].type === 'identifier' &&
        ...
        for (let idx = 0; idx < term.elements[1].elements.length; idx += 2) {
          ...
          const {success, value} = evaluate (env_)
                                            (term.elements[1].elements[idx + 1])
                                            (value => ({success: false, value}))
                                            (value => ({success: true, value}));
          if (!success) return reject (value);
          env_ = {...env_, [name]: value};
        }
        return evaluate (env_) (term.elements[2]) (reject) (resolve);
      }
      if (term.elements[0].type === 'identifier' &&
        ...
        return resolve (
          params.reduce (
            ...
            args => evaluate (params.reduce ((env, param, idx) =>
                                               ({...env, [param]: args[idx]}),
                                             env))
                             (term.elements[2])
                             (err => { throw err; })
...
export const evaluateFile = env => filename => reject => resolve => {
  fs.readFile (filename, {encoding: 'utf8'}, (err, src) => {
    if (err != null) {
      ...
      parse (src)
            (reject)
            (value => _ => evaluate (env) (value) (reject) (resolve));
    }
  });
};
...
//  succeeds ({map: f => xs => xs.map (x => f (x))}) ('(map :z [{:x 1 :y 2} {:x 3 :y 4}])') ([undefined, undefined]);
    ...
    succeeds ({'true': true, 'false': false}) ('true') (true);
    succeeds ({'true': true, 'false': false}) ('false') (false);

    succeeds ({'true': true, 'false': false}) ('(if true 8 x)') (8);
    succeeds ({'true': true, 'false': false}) ('(if false x 8)') (8);
    fails ({'true': true, 'false': false}) ('(if "xxx" x x)') (new TypeError ('Predicate evaluated to non-Boolean value'));
  }
}
