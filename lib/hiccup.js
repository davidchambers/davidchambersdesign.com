...
const attempt = thunk => reject => resolve => {
  try {
...
const parse = input => reject => resolve => {
  if (input === '') {
    ...
    return attempt (() => JSON.parse (raw))
                   (_ => reject (new SyntaxError ('Invalid string literal')))
                   (value => resolve ({type: 'string-literal', value})
...
if (url.fileURLToPath (import.meta.url) === process.argv[1]) {
  ...
  { // evaluate
    ...
    succeeds ({'Math.sqrt': Math.sqrt, '*': (x, y) => x * y}) ('(Math.sqrt (* 8 8))') (8);
  }
}
