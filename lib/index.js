...
const yearMonth = dt => dt.toFormat ('y-MM');
...
const program = pipe ([
  ...
  map (groupBy (on (equals) (B (yearMonth) (prop ('datetime'))))),
  map (map (B (mapLeft (([post]) => post.datetime.toFormat ('MMMM y')))
              (join (Pair)))),
  map (map (([month, posts]) =>
              $li ({})
                  ([h2 ({}) ([month]),
                                   ...
                                   ([a ({href: `/TK/${post.slug}`}) ([post.title]),
                                     ...
                                     time ({datetime: post.datetime.toISO()})
                                          ([post.datetime.toFormat ('d MMMM y | h:mm') +
                                            toLower (post.datetime.toFormat ('a'))])]))
                             (posts))]))),
