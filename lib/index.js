...
const $ = tagName => attrs => children => ({type: 'multi-line', tagName, attrs, children});

//    _ :: String -> StrMap String -> Array Node -> Node
const _ = tagName => attrs => children => ({type: 'single-line', tagName, attrs, children});

//    x :: String -> StrMap String -> Node
const x = tagName => attrs => ({type: 'self-closing', tagName, attrs});
...
export const render = indent => level => inline => (
  foldMap (String)
          (node => {
             ...
             switch (node.type) {
               case 'self-closing':
                 return indentation + '<' + node.tagName + attrs + ' />\n';
               case 'single-line':
                 return indentation + '<' + node.tagName + attrs + '>' +
