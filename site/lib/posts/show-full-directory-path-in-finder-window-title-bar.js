...
const excerpt = [p(["If you have ever found yourself command-clicking the ", "title of a Finder window to find out where you are (", strong(["/Library/Fonts"]), " or ", strong(["~/Library/Fonts"]), " ", "is one I've double-checked many times), you'll understand how ", "pleased I was to discover that there is a command which can be ", "entered in Terminal to ", a({
...
const body = [...excerpt, captioned$002Dimages([{
  ...
  caption: [strong(["Before:"]), " ", "Finder windows with directory name only in title bar"]
}]), p(["In the screenshot above, it is clear that the two Finder windows are ", "displaying different Fonts folders, but it is unclear which is which."]), p(["To display the full path, copy and paste the following into Terminal ", "and hit ", strong(["return"]), "."]), code$002Dblock("console")("defaults write com.apple.finder _FXShowPosixPathInTitle -bool YES\n  "), p(["For the changes to take effect, you will need to restart Finder:"]), code$002Dblock("console")("killall Finder\n  "), captioned$002Dimages([{
  alt: "Finder windows with paths",
  src: "/images/posts/2/finder-windows-with-paths.png",
  caption: [strong(["After:"]), " ", "Finder windows with full directory path in title bar"]
