import {p, ul, li, strong} from "../elements.js";
import {captioned$002Dimages, code$002Dblock} from "../components.js";
import datetime from "../datetime.js";
const excerpt = [p(["I've recently become interested in optimizing sites for ", "the iPhone and iPod touch. While nothing beats testing on ", "the device itself, I often find it quicker to test changes ", "on my Mac. Changing the user agent string is a piece of cake ", "in Safari (Develop > User Agent > Mobile Safari) but what ", "about adjusting the browser window's dimensions to match ", "those of the iPhone?"]), p(["I've created two bookmarklets to allow the current page to ", "be loaded in an iPhone-sized window with a single click:"]), ul([li([p([strong(["Portrait"]), " (labelled \"⁑\")"]), code$002Dblock("plain-text")("javascript:open(location,'iPhone:portrait','innerWidth='+(320+15)+',innerHeight='+(480+15)+',scrollbars=yes');\n      ")]), li([p([strong(["Landscape"]), " (labelled \"**\")"]), code$002Dblock("plain-text")("javascript:open(location,'iPhone:landscape','innerWidth='+(480+15)+',innerHeight='+(320+15)+',scrollbars=yes');\n      ")])]), captioned$002Dimages([{
...
}]), p(["It appears that I need to rework this site's style sheet ", "to better present content in these smaller viewports!"])];
export default {
  id: 37,
