...
`), p(["The Ruby code (the one beginning with ", code(["ids.map"]), ") reads top\n    to bottom and is easy to follow. The Python code is equally succinct but\n    takes a bit of effort to decipher."]), p(["I've been greatly enjoying the act of writing JavaScript lately,\n    so simply for pleasure I worked out the JavaScript equivalent."]), p(["My first attempt used the ", code(["filter"]), " array method."]), code$002Dblock("javascript")(`ids.filter(function (id) {
    var obj = repository.retrieve(id);
    return obj && obj.name;
}).join('\\n');
`), p([a({
  href: "https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Array/Filter"
})([code(["filter"])]), ", though, just removes from an array the\n    items which fail the provided \"test\". So the code above is\n    on the right track, but fails to produce a list of names."]), p([a({
  href: "https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Array/Reduce"
})([code(["reduce"])]), " is the correct method for the job. ", code(["reduce"]), " \"reduces\" an array to a single value, which\n    could be a string, an object, another array", $2014, "whatever!"]), p(["Note the empty array (", code(["[]"]), ") on line 5 – that's our\n    \"accumulator\"."]), code$002Dblock("javascript")(`ids.reduce(function (ids, id) {
    var obj = repository.retrieve(id);
