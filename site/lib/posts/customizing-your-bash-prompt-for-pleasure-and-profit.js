...
const excerpt = [
  ...
  p([
    'Since only the current directory name is visible, I find myself running ',
    code(['pwd']),
    ' more often than is healthy. Also, I find the uneven prompt\n    length jarring.'
  ])
];
const body = [
  ...excerpt,
  p([
    'My solution to both of these problems is to include the full path ',
    em(['on its own line']),
    ' (with a preceding ',
    code(['\\n']),
