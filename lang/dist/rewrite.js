...
const {_apply, apply, chain, concat, const_, construct, filter, flip, map, not, reject} = Prelude$1;
...
const rewriteModule = ({imports, exports, statements}) => namesExportedFrom => Prelude$1.chain(imports$0027 => (names => (preludeIdent => (names$0027 => (rewrite => (preludeDefinition => (preludeDestructuring => Future.resolve(Module({
...
const rewriteImportDeclaration = namesExportedFrom => importDeclaration => (() => {
  switch (importDeclaration.type) {
    case "ImportDeclaration":
      return Future.resolve(importDeclaration);
