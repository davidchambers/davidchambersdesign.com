...
function rewriteNode(node) {
  switch (node.type) {
    ...
    case 'ArrowFunctionExpression': {
      return (
        Serif.ArrowFunctionExpression(node.parameters.map(rewriteNode))
                                     (rewriteNode(node.body))
      );
    }
    case 'PropertyAccessor': {
      return (
        Serif.ArrowFunctionExpression([Serif.Identifier('x')])
                                     (Serif.MemberExpression(Serif.Identifier('x'))
                                                            (Serif.StringLiteral(node.identifier.name)))
      );
    }
    ...
    case 'UnaryExpression': {
      return (
        Serif.UnaryExpression(node.operator)
                             (rewriteNode(node.argument))
      );
    }
    case 'BinaryExpression': {
      return (
        Serif.BinaryExpression(node.operator)
    ...
    case 'MapExpression': {
      return (
        Serif.CallExpression(Serif.CallExpression(Serif.MemberExpression(Serif.Identifier('Prelude'))
                                                                        (Serif.StringLiteral('map')))
                                                 ([rewriteNode(node.left)]))
                            ([rewriteNode(node.right)])
      );
    }
    ...
    case 'LogicalExpression': {
      return (
        Serif.LogicalExpression(node.operator)
                               (rewriteNode(node.left))
                               (rewriteNode(node.right))
      );
    }
    case 'ConditionalExpression': {
      return (
        Serif.ConditionalExpression(rewriteNode(node.predicate))
    ...
    case 'CallExpression': {
      return (
        Serif.CallExpression(rewriteNode(node.callee))
                            (node.arguments.map(rewriteNode))
      );
    }
    case 'VariableDeclaration': {
      return (
        Serif.VariableDeclaration(rewriteNode(node.pattern))
                                 (rewriteNode(node.expression))
      );
    }
    case 'FunctionDeclaration': {
      return (
        Serif.FunctionDeclaration(node.name)
